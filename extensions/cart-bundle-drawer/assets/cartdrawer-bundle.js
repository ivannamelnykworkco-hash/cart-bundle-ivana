...
(() => {
  const OPENERS = [
    '[data-cart-toggle]', '[data-action="open-drawer"][data-drawer-id*="cart"]',
    '[aria-controls*="cart"][data-action="open"]', '.cart-toggle', '.header__cart', '.open-cart-btn',
    'button[name="cart"]', 'a[href="#cart-drawer"]', '.js-mini-cart-open', '.icon-cart', '.site-header__cart'
  ];
  const THEME_DRAWERS = ['cart-drawer', '.cart-drawer', '#CartDrawer', '.drawer--right', '.mini-cart', '.cart-sidebar'];
  ...
  const productImg = (src) => src ? src.replace(/(\.jpg|\.jpeg|\.png|\.webp)(?!\?)/i, '$1?width=128') : '';
  ...
  function render(cart) {
    ...
    if (!cart.items?.length) {
      lineList.innerHTML = '<p role="status">Your cart is empty.</p>';
    ...
    cart.items.forEach(it => {
      ...
      const compare = (Number.isFinite(it.compare_at_price) && it.compare_at_price > current)
        ? it.compare_at_price
        : null;
      ...
      row.innerHTML = `
        ...
        <div>
          <p class="xcart-line__title">${it.product_title}</p>
          <div class="xcart-line__meta">${it.variant_title || ''}</div>
          <button class="xcart-remove" data-key="${it.key}" aria-label="Remove ${it.product_title}">Remove</button>
        </div>
        <div>
          <div class="xcart-qty" aria-label="Quantity for ${it.product_title}">
            ...
            <input type="number" class="xcart-qty-input" min="0" step="1" value="${it.quantity}" data-key="${it.key}" inputmode="numeric" />
          ...
          <div class="xcart-line-total" style="text-align:right;margin-top:6px">
  ...
  async function changeQuantityByKey(key, quantity) {
    if (!key) return;
    const q = Math.max(0, Math.min(999, Number.isFinite(+quantity) ? +quantity : 0));
  ...
  function interceptAddToCart() {
    document.addEventListener('submit', async (e) => {
      ...
      e.preventDefault();
      try {
        await fetch('/cart/add.js', { method: 'POST', headers: { 'Accept': 'application/json' }, body: new FormData(form) });
      } catch (err) { console.warn('add.js error', err); }
      openDrawer();
    });
    ...
    document.addEventListener('ajaxProduct:added', openDrawer, { passive: true });
    document.addEventListener('theme:cart:updated', openDrawer, { passive: true });
  ...
  if (checkoutBtn && !checkoutBtn.__xcartBound) {
    ...
    checkoutBtn.addEventListener('click', async (e) => {
      e.preventDefault();
      await ensureFreshCart();
      setTimeout(() => { window.location.assign('/checkout'); }, 50);
    });
  }

  // ---- Events ----
  document.addEventListener('click', (e) => {
    if (e.target.closest('[data-xcart-close]') || e.target === overlay) { closeDrawer(); return; }

    const inc = e.target.closest('.xcart-qty-inc');
    if (inc) {
      const key = inc.getAttribute('data-key');
      const input = lineList.querySelector(`.xcart-qty-input[data-key="${CSS.escape(key)}"]`);
      changeQuantityByKey(key, (parseInt(input?.value || '0', 10) || 0) + 1);
      return;
    }
    ...
    if (dec) {
      ...
      const input = lineList.querySelector(`.xcart-qty-input[data-key="${CSS.escape(key)}"]`);
      changeQuantityByKey(key, Math.max(0, (parseInt(input?.value || '0', 10) || 0) - 1));
      return;
    }
    const rm = e.target.closest('.xcart-remove');
    if (rm) { removeByKey(rm.getAttribute('data-key')); return; }

    if (e.target.matches('#xcart-apply-discount')) {
      const code = (document.getElementById('xcart-discount-input')?.value || '').trim();
      if (code) window.location.assign('/discount/' + encodeURIComponent(code) + '?redirect=/cart');
    }
  });

  document.addEventListener('change', (e) => {
    if (e.target.matches('.xcart-qty-input')) {
  ...
  // ---- Bootstrap ----
  function init() { bindOpeners(); hideThemeDrawers(); interceptAddToCart(); }
  const mo = new MutationObserver(() => { bindOpeners(); hideThemeDrawers(); });

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { init(); mo.observe(document.documentElement, { childList: true, subtree: true }); });
  } else {
    init(); mo.observe(document.documentElement, { childList: true, subtree: true });
  }
})();
